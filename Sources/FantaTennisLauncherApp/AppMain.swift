...
final class FantaTennisLauncherAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private let runtimeButton = NSButton(title: "Get Free Runtime", target: nil, action: nil)
    ...
    @objc private func launchGame() {
        ...
        let status = LauncherInstaller.runtimeStatus()
        guard case let .ready(runtime) = status else {
            refreshRuntimeStatus()
            if status == .sikarugirNeedsEngine {
                append("Sikarugir Creator is installed, but no runnable Wine engine was found.")
                append("Opening Sikarugir Creator so you can create a free wrapper or install an engine.")
                openSikarugirCreator()
            } else {
                append("Install Sikarugir or Wine, then press Launch again.")
                openRuntimeDownload()
            }
            return
        }
